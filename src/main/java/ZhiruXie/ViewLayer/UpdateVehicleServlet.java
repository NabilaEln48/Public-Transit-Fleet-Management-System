...
public class UpdateVehicleServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!isBlank(vehicleId)) {
            try (Connection con = DataSource.getConnection("cst8288", "cst8288")) {
                ...
                if (vehicle != null) {
                    formValues.put("vehicleId", vehicle.getId());
                    formValues.put("registrationNumber", vehicle.getRegistrationNumber());
                    formValues.put("fuelUsed", vehicle.getFuelUsed());
                    formValues.put("efficiencyRate", String.valueOf(vehicle.getEfficiencyRate()));
                    formValues.put("capacity", String.valueOf(vehicle.getCapacity()));
                    formValues.put("assignedRoute", vehicle.getAssigned_route());
                    formValues.put("category", getCategoryDisplayName(vehicle.getCategory().name()));
                    formValues.put("operationalState", vehicle.getOperationalState().name());
                } else {
                    formErrors.put("general", "Vehicle with ID " + vehicleId + " not found.");
                }
            } catch (Exception e) {
                e.printStackTrace();
        ...
        if (formSubmitted != null) {
            ...
            if (formErrors.isEmpty()) {
                ...
                try (Connection con = DataSource.getConnection("cst8288", "cst8288")) {
                    ...
                    if (success) {
                        response.sendRedirect("UpdateVehicle?success=1");
                        return;
                    } else {
                        formErrors.put("general", "Failed! Vehicle Does Not Exist.");
                    }
    ...
    private void printForm(HttpServletRequest request, HttpServletResponse response,
        ...
        // This is a mapping from Enum value to display name for the category dropdown
        Map<String, String> categoryEnumToDisplayMap = new HashMap<>();
        categoryEnumToDisplayMap.put("DieselBus", "Diesel Bus");
        categoryEnumToDisplayMap.put("ElectricLightRail", "Electric Light Rail");
        categoryEnumToDisplayMap.put("DieselElectricTrain", "Diesel-Electric Train");
        
        try (PrintWriter out = response.getWriter()) {
            out.println("<!DOCTYPE html>");
            out.println("<html><head><title>Update Vehicle</title>");
            ...
            if ("1".equals(request.getParameter("success"))) {
                out.println("<div class='success-message'>Vehicle Updated Successfully!</div>");
            } else if (errors.containsKey("general")) {
                out.printf("<div class='failure-message'>%s</div>", errors.get("general"));
            }

            out.println("<form method='post' action='UpdateVehicle'>");
            out.println("<input type='hidden' name='formSubmitted' value='true'/>");
            
            // Vehicle ID (Read-only for update)
            out.println("<div>");
            out.println("<label>Vehicle ID:</label>");
            out.printf("<input type='text' name='vehicleId' value='%s' readonly/>", values.getOrDefault("vehicleId", ""));
    ...
    // Helper method to convert enum name to display name
    private String getCategoryDisplayName(String enumName) {
        switch (enumName) {
            case "DieselBus": return "Diesel Bus";
            case "ElectricLightRail": return "Electric Light Rail";
            case "DieselElectricTrain": return "Diesel-Electric Train";
